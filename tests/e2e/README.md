...
The hermetic pair touches nothing: the fixtures never leave the machine, and
`desktop-setup.js` runs the server against a temporary `HOME` and a temporary
notes directory, so it cannot see the developer's notes or credentials.
...
| `EVERFREE_PYTHON` | `python3` | `desktop-setup.js` |
| `EVERFREE_TEST_PORT` | `52398` | `desktop-setup.js` |
